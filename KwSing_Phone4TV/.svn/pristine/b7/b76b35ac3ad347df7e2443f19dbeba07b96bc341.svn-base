 ...
 */
package cn.kuwo.sing.phone4tv.view.widget;
...
public class ChangeLog {
    ...
    public static final String DEFAULT_CSS =
            ...
            "li { margin-left: 0px; font-size: 1.2em;}" + "\n" +	
            "ul { padding-left: 2em; }";
    
    public static final String DEFAULT_CSS_EX =
    		"body {color:#6b8395;}" + "\n" + 
            "h1 { margin-left: 0px; font-size: 2.0em; }" + "\n" +
            "li { margin-left: 0px; font-size: 1.5em;}" + "\n" +	
            "ul { padding-left: 2em; }";
    
    public static final String DEFAULT_CSS_EX_DENSITY_2 =
    		"body {color:#6b8395;}" + "\n" + 
    				"h1 { margin-left: 0px; font-size: 1.5em; }" + "\n" +
    				"li { margin-left: 0px; font-size: 1.125em;}" + "\n" +	
    				"ul { padding-left: 1.5em; }";
    
    public static final String DEFAULT_SMALL_CSS =
            "h1 { margin-left: 0px; font-size: 1.2em; }" + "\n" +
            "li { margin-left: 0px; }" + "\n" +
            "ul { padding-left: 2em; }";
    
    public static final String DARK_THEME_CSS =
            "body { color: #ffffff; background-color: #ffffffff; }" + "\n" + DEFAULT_CSS;
    ...
    public AlertDialog getLogDialog4Update(String formateLog) {
    	return getDialog(formateLog);
    }
    ...
    protected AlertDialog getDialog(boolean full) {
        ...
        wv.loadDataWithBaseURL(null, getFullLog(), "text/html", "UTF-8", null);
    ...
    protected AlertDialog getDialog(String formateLog) {
        WebView wv = new WebView(mContext);
        wv.setBackgroundColor(Color.WHITE);
        wv.loadDataWithBaseURL(null, formateLog, "text/html", "UTF-8", null);

        AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
        builder.setTitle(mContext.getResources().getString(R.string.changelog4update_title))
                .setView(wv)
                .setCancelable(false);
        return builder.create();
    }
    ...
    public String getNetworkErrorFormatLog(String content) {
    ...
    public String getFormatLog(String version, String time, String content)
    {
    	StringBuilder sb = new StringBuilder();
        ...
        String strCurVerCode = Integer.toString(getCurrentVersionCode());
        String strUpdateVerCode = version.replaceAll("\\.", "");
        int length = strCurVerCode.length() > strUpdateVerCode.length() ? strUpdateVerCode.length() : strCurVerCode.length();
        strUpdateVerCode = strUpdateVerCode.substring(0, length);
        strCurVerCode = strCurVerCode.substring(0, length);
        int nUpdateVerCode = Integer.parseInt(strUpdateVerCode);
        int nCurVerCode = Integer.parseInt(strCurVerCode);
        if (nCurVerCode>=nUpdateVerCode)
        {
        	sb.append("<h1>");
        	sb.append(String.format("当前版本为：%s，已经是最新版本", getCurrentVersionName()));
        	sb.append("</h1>");
        }
        else
        {
            sb.append("<h1>");
            sb.append(String.format("发现新版本：%s （更新日期：%s）", version, time));
            sb.append("</h1><ul>");
        	sb.append(content);
        	sb.append("</ul>");
        }
    ...
    protected SparseArray<ReleaseItem> getMasterChangeLog(boolean full) {
        return readChangeLogFromResource(R.xml.changelog_detail, full);
    }
